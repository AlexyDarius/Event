...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Ensure you use prepared statements to prevent SQL injection / place = ?, date = ?, text = ?, link = ?
    $sql = "UPDATE dariusdev_event SET title = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("si", $editedTitle, $eventId);
